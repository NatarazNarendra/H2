 ...
 *//**
	 ...
	 *//*
		 ...
		 * 
		 * @DataJpaTest public class OrderRepositoryTest {
		 * 
		 * @Autowired private OrderRepostiroty orderRepostiroty;
		 * 
		 * 
		 * @Test public void test_SaveOrder() {
		 * 
		 * Order order = new Order(); order.setId("1"); order.setDescription("desc");
		 * order.setName("name"); orderRepostiroty.save(order);
		 * 
		 * Order savedOrder = orderRepostiroty.findById("1").get();
		 * 
		 * assertNotNull(savedOrder); assertEquals(savedOrder.getName(),
		 * order.getName()); }
		 * 
		 * @Test(expected = NoSuchElementException.class) public void
		 * test_OrderIsNotPresent() {
		 * 
		 * Order order = new Order(); order.setId("1"); order.setDescription("desc");
		 * order.setName("name"); orderRepostiroty.save(order);
		 * 
